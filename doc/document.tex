...
\begin{proposition}
  ...
  then there exists $V \in \mathbb{C}^{M \times M}$ such that
  \begin{equation*}
    \mathbf{g}(x) = V \Phi(x),
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    = B\Psi(x),
  \end{equation}
  where $B = [\mathbf{b}_1,\cdots,\mathbf{b}_N]^T$.
...
\end{equation*}
Combined with (\ref{eq:koopman-mode-decomposition}) we obtain
\begin{equation*}
  x(n) = \sum\limits_{k = 1}^M v_k \mu_k^n \varphi_k(x(0)),
\end{equation*}
which means we can predict the whole trajectory.

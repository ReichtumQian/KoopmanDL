...
class TanhResNet(torch.nn.Module):
  ...
  def __init__(self, n_input, n_output, hidden_layer_sizes):
    ...
    for i, hidden_size in enumerate(hidden_layer_sizes):
      if i == 0:
        layers.append(torch.nn.Linear(n_input, hidden_size))
      else:
...
class TanhResNetWithNonTrainable(TanhResNet):
  ...
  def _save_to_state_dict(self, destination):
    return super()._save_to_state_dict(destination)
